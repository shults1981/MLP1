...
{
    int X;           // размерногсть входного вектора
    int Y;           // размерность входного вектора
    int L;           // кол-во слоёв
    int *J;          // кол-во нейронов в скпытых слоях кроме последнего
    double *Xin;     // входной вектор
    double *Yout;    // выходной вектор
    double ***x;     // входные сигналы нейрона
    double ***w;     // весовые коэфициенты нейрона
    double **NET;    // взвешенная сумма нейрона
    double **Out;    // выход  нейронов
    double **O;      // пороговый уровень нейрона(смещение)
    int FofA;        // номер функции активации

    double deltaX;   //  вспомогательные переменные:
    double deltaY;   //  для нормализации
    double alphaX;   //  входных и выходных
    double alphaY;   //  данных
} MLP;


struct  trVec    // структура обучающего вектора
{
    double *Xin;  // входной вектор
    double *Yout; // выходной вектор
};
...
//-------------------------------ПРОТОТИПЫ ФУНКЦИЙ----------------------------------------------
void main_menu (MLP*);                  //меню выбора из какой структуры работать новая/сохраненная в файл
int op_mode_menu(void);                 // меню выбора Работа/Обучение/Сохранение
int tr_mode_menu(void);                 // меню выбора:-создать файл с обуч данными/
                                        //             -посмотреть обучающие данные
                                        //             -начать обучение
void initialization_MLP_new ( MLP *);   // инициализация нового МСП с вводом основных параметров
void initialization_MLP_File ( MLP *);  // инициализация сохраненного МСП с из файла
int  work_MLP(MLP*);                    // работа МСП в режиме расчета
void basic_calc_MLP(MLP *, double*);    // основной расчет МСП(вычисление выхода по входу)
void training_MLP(MLP*);                   // обучения МСП
void save_MLP_File ( MLP *);            // сохранение МСП в файл

void delete_MLP (MLP *);                // удаление многослойного пересептрона из памяти
void creat_train_data_file (MLP*);      // создание файла с обучающим вектором
void output_train_data(MLP*);           // просмотр обучающих данных
int  num_control (char *,int, int, int, int); // ф-ия проверки на:
                                             //  -число(целое/вещ.определяется перем DorI)
                                             //  -принадлежность интервалу (min;max)
                                             //  -принадлежность к отрицательным перем SorUS (min задавать <0)
double (*funk_of_act) (double);      // указатель на функцию активации
double (*Der_funk_of_act)(double);   // указатель на производную ф-ии активации

double sigmoid (double);             // функция активации сигмоид(логистическая)
double gip_tan (double);             // функция активации гиперболический тангенс
double Der_sigmoid (double);         // производная функции активации сигмоид
double Der_gip_tan (double);         // производная функции активации гиперболический тангенс
void rating01 (struct trVec *tV, int M, int X, int Y, MLP *);// нормализация вектора к интервалу [0;1]
void rating11 (struct trVec *tV, int M, int X, int Y, MLP *); // нормализация вектора к интервалу [-1;1]
double RMS_error (MLP*,struct trVec *, int,int,int);// среднеквадратическая ошибка
void mixing_trVec(struct trVec *,int, int ,int); // перемешивание обучающего вектора
...
//------------------------------------------------ЯДРО---------------------------------------
 ...
 for(;;)
 {
    ...
    main_menu(P1); //выбор способа инициализации МСП


#ifdef DEBUG //------------------------------отладочная информация---------------------------
    ...
    while ((iMenu1=op_mode_menu())!=4) //большой цикл выбора ОБУЧЕНИЕ/РАБОТА/СОХРАНЕНИЕ
    {
        ...
        case 1:
        {
            while((iMenu2=tr_mode_menu())!=4)// малый цикл выбора в меню обучения
                switch (iMenu2)
                {
        ...
        case 2:
        {
            while (work_MLP(P1)!=0)// цикл работа в режиме работа
            {
